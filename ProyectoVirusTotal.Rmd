...
test <- fviz_cluster(test_kmeans, data = t(json_df_filtrados[,4:45]) , geom=c("point", "text") )

ggplotly(test)

plotcluster(t(json_df_filtrados[,4:45]), test_kmeans$cluster)



plot(t(json_df_filtrados[,4:45]),col=test_kmeans$cluster) 
points(test_kmeans$center,col=1:10,pch=8,cex=1)            
```
...
El primer método de análisis de datos que voy a aplicar es FCA
...
b <- fc_detected$concepts[1:200]
implicaciones <-  fc_detected$implications$support()
implicaciones
```
...
tengo que buscar caracterizadores de los archivos para poder hacer un modelo de predicción.tengo que buscar caracterizadores de los archivos para poder hacer un modelo de predicción.
